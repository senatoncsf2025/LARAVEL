<?php
    ...
    class UserExternoController extends Controller
    {
        ...
        // 📌 Reporte en PDF (con filtros y campos dinámicos)
        public function reporte(Request $request, $rol)
        {
            $query = UserExterno::with(['vehiculo', 'pc'])
                ->where('rol_externo', $rol);
            ...
            // 📑 Campos seleccionados (si no vienen, usar algunos por defecto)
            $campos = $request->input('campos', ['nombre', 'apellido', 'email']);

            // 📄 Generar PDF con DomPDF
            $pdf = Pdf::loadView("reportes.usuarios", compact('usuarios', 'campos', 'rol'));
            return $pdf->stream("reporte_{$rol}.pdf");
        }

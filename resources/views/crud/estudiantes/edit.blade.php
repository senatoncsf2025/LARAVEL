    ...
    <form action="{{ route('estudiantes.update', $estudiante->id) }}" method="POST">
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="text" name="apellido" class="form-control" value="{{ old('apellido', $estudiante->apellido) }}">
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="trae_vehiculo" id="trae_vehiculo" class="form-control">
                <option value="0" {{ old('trae_vehiculo', $estudiante->trae_vehiculo) == 0 ? 'selected' : '' }}>No</option>
                <option value="1" {{ old('trae_vehiculo', $estudiante->trae_vehiculo) == 1 ? 'selected' : '' }}>Sí</option>
            </select>
        </div>

        <div id="vehiculoFields" style="{{ old('trae_vehiculo', $estudiante->trae_vehiculo) ? 'display:block;' : 'display:none;' }}">
            <div class="row">
                <div class="col-md-3 mb-3">
                    <label for="placa" class="form-label">Placa</label>
                    <input type="text" name="placa" id="placa" class="form-control" value="{{ old('placa', $estudiante->placa) }}">
                </div>
                <div class="col-md-3 mb-3">
                    <label for="marca" class="form-label">Marca</label>
                    <input type="text" name="marca" id="marca" class="form-control" value="{{ old('marca', $estudiante->marca) }}">
                </div>
                <div class="col-md-3 mb-3">
                    <label for="modelo" class="form-label">Modelo</label>
                    <input type="text" name="modelo" id="modelo" class="form-control" value="{{ old('modelo', $estudiante->modelo) }}">
                </div>
                <div class="col-md-3 mb-3">
                    <label for="color" class="form-label">Color</label>
                    <input type="text" name="color" id="color" class="form-control" value="{{ old('color', $estudiante->color) }}">
                </div>
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="trae_pc" id="trae_pc" class="form-control">
                <option value="0" {{ old('trae_pc', $estudiante->trae_pc) == 0 ? 'selected' : '' }}>No</option>
                <option value="1" {{ old('trae_pc', $estudiante->trae_pc) == 1 ? 'selected' : '' }}>Sí</option>
            </select>
        </div>

        <div id="pcFields" style="{{ old('trae_pc', $estudiante->trae_pc) ? 'display:block;' : 'display:none;' }}">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <label for="codigo_pc" class="form-label">Código PC</label>
                    <input type="text" name="codigo_pc" id="codigo_pc" class="form-control" value="{{ old('codigo_pc', $estudiante->codigo_pc) }}">
                </div>
                <div class="col-md-6 mb-3">
                    <label for="serial_pc" class="form-label">Serial PC (últimos 4 dígitos)</label>
                    <input type="text" name="serial_pc" id="serial_pc" maxlength="4" class="form-control" value="{{ old('serial_pc', $estudiante->serial_pc) }}">
                </div>
            </div>
        </div>
